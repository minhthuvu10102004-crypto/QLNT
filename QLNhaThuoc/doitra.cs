...
namespace QLNhaThuoc
{
    public partial class doitra : XtraForm
    {
        ...
        private void doitra_Load(object sender, EventArgs e)
        {
            ...
            gridViewphieutrahang.OptionsView.ShowIndicator = false;
            //
            gridViewphieutrahang.OptionsBehavior.Editable = true; // Cho phép chỉnh
            gridViewphieutrahang.OptionsBehavior.ReadOnly = false;
            ...
            gridViewphieutrahang.Columns["TrangThai"].OptionsColumn.AllowEdit = true;
            ...
            gridViewphieutrahang.Appearance.Row.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Far;
            ...
            gridControlphieudoitra.RepositoryItems.Add(repoTrangThai);
            gridViewphieutrahang.Columns["TrangThai"].ColumnEdit = repoTrangThai;
            ...
            //
            this.gridViewphieutrahang.CellValueChanged += gridViewnhaplieu_CellValueChanged;
            this.gridViewphieutrahang.ShowingEditor += gridViewphieutrahang_ShowingEditor;
            this.gridViewphieutrahang.RowCellStyle += gridViewnhaplieu_RowCellStyle;
        }
        ...
        private void gridViewnhaplieu_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (e.Column.FieldName == "TrangThai")
            {
                ...
                string maPhieuTra = view.GetRowCellValue(e.RowHandle, "MaPhieuTra")?.ToString();
                if (string.IsNullOrEmpty(maPhieuTra)) return;

                // Nếu phiếu đã xác nhận thì không làm gì
                if (newStatus == "Đã xác nhận") return;
                    ...
                    if (result == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            using (SqlConnection conn = new SqlConnection(constr))
                            {
                                ...
                                using (SqlCommand cmd = new SqlCommand("usp_TraNCC_CapNhatKho", conn))
                                {
                                ...
                                using (SqlCommand cmdUpdate = new SqlCommand("UPDATE PhieuTraHang SET TrangThai = N'Đã xác nhận' WHERE MaPhieuTra = @MaPhieuTra", conn))
                                {
                                    cmdUpdate.Parameters.AddWithValue("@MaPhieuTra", maPhieuTra);
        ...
        private void gridViewphieutrahang_ShowingEditor(object sender, CancelEventArgs e)
        {
            GridView view = sender as GridView;
        ...
        private void btnsua_Click(object sender, EventArgs e)
        {
            ...
            // MỞ FORM CHỈ 1 LẦN
            using (chitietphieudoitra form = new chitietphieudoitra(chitietphieudoitra.Mode.ChinhSua, maPhieu))
            {
                form.Owner = this; // Đặt form cha
                if (form.ShowDialog() == DialogResult.OK)
                {
                    LoadPhieuTraHangData(); // reload dữ liệu sau khi lưu
                }
            }
